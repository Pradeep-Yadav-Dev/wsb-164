...
export default function Header() {
    ...
    let { storeCat } = useContext(Econtext)
                    ...
                    <div className="menu">
                        <nav>
                            <ul className="d-flex justify-content-center">
                                ...
                                {storeCat?.map((v) =>
                                    <li key={v._id}>
                                        <Link href={`/categories/${v.slug}`}>
                                            {v.parentCategoryName} {v.childOne?.length > 0 && <FaAngleDown />}
                                        </Link>
                                        ...
                                        {v.childOne?.length > 0 && (
                                            <ul className="mega_menu d-flex">
                                                {v.childOne.map((sub) => (
                                                    <li key={sub._id}>
                                                       <Link href={`/categories/${v.slug}/${sub.slug}`}>{sub.subCategoryName}</Link>
                                                        {sub.childTwo?.length > 0 && (
                                                            <ul>
                                                                {sub.childTwo.map((child) => (
                                                                    <li key={child._id}>
                                                                        <Link href={`/categories/${v.slug}/${sub.slug}/${child.slug}`}>
